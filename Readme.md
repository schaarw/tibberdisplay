...
* The type of the CYD is(`CYD ESP32 Sunton 2432S028` (See details here: https://github.com/rzeldent/platformio-espressif32-sunton)


# Configuration 

## Secrets: 
* Edit `secrets_example.yaml` with your wifi credentials and tibber auth token and rename it to `secrets.yaml`. It should look like this: 

    ```
    wifi_ssid: "ReplacemeWifi"
    wifi_password: "SecretPassword123"
    tibber_auth_token_value: "TibberAuthToken"
    ```    
...
## Tibber token
* This project needs your *tibber token* to run. Please fetch it from your tibber account via developer tools. You *can* provide this token initially via `secrets.yaml` - this will build it and allows a quick init, or set this token later on via web ui. 
...
* Within this cloned github repository, do a `esphome run tibberdisplay.yaml` to build & deploy the tibber application onto your device via USB to serial connection. 
...
# Support the orignial creator of this code 
